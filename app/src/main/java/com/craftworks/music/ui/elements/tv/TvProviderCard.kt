...
fun NavidromeProviderCard(
    ...
    val checked by remember { derivedStateOf { server.enabled && server.id == currentServerId } }
...
fun EmbyJellyfinProviderCard(
    ...
    ListItem(
                ...
                if (libraries.size > 1 && server.id == currentServerId) {
                    Row(
                        ...
                        libraries.forEach { (library, isSelected) ->
                            FilterChip(
                                onClick = {
                                    EmbyJellyfinManager.currentServerId.value?.let { serverId ->
        ...
        onClick = {
            val newChecked = !checked
            EmbyJellyfinManager.setServerEnabled(server.id, newChecked)
            if (newChecked) {
                coroutineScope.launch {
                    AppearanceSettingsManager(context).setUsername(server.username)
                }
            }
            Log.d("EMBY_JELLYFIN", "Emby Server ${server.id} enabled: $newChecked")
        },
        onLongClick = {
            EmbyJellyfinManager.removeServer(server.id)
            DataRefreshManager.notifyDataSourcesChanged()
        }
    )
}
